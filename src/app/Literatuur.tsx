import React, {Component} from 'react';

class Literatuur extends Component<any, any> {
    render() {
        return (
            <main>
                <p>
                    Coventry, L. M., Jeske, D., Blythe, J. M., Turland, J., & Briggs, P. (2016). Personality and social framing in privacy decision-making: A study on cookie acceptance. Frontiers in psychology, 7, 1341.
                </p>
                <p>
                    Iyengar, S. S., & Lepper, M. R. (2000). When choice is demotivating: Can one desire too much of a good thing?. Journal of personality and social psychology, 79(6), 995.
                </p>
                <p>
                    Kierkegaard, S. M. (2005). How the cookies (almost) crumbled: Privacy & lobbyism. Computer Law & Security Review, 21(4), 310-322.
                </p>
                <p>
                    Miyazaki, A. D. (2008). Online privacy and the disclosure of cookie use: Effects on consumer trust and anticipated patronage. Journal of Public Policy & Marketing, 27(1), 19-33.
                </p>
                <p>
                    Sipior, J. C., Ward, B. T., & Mendoza, R. A. (2011). Online privacy concerns associated with cookies, flash cookies, and web beacons. Journal of internet commerce, 10(1), 1-16.
                    https://www.tandfonline.com/doi/pdf/10.1080/15332861.2011.558454
                </p>
                <p>
                    Susser, D., Roessler, B., & Nissenbaum, H. (2018). Online manipulation: Hidden influences in a digital world.
                </p>
            </main>
        );
    }
}

export default Literatuur;
